      ...
      <el-table ref="roleTable"
        ...
        <el-table-column label="客户头像" align="center">
          <template slot-scope="scope">
            <el-image :preview-src-list="scope.row.avatarUrl.indexOf('http')>-1?
            [...scope.row.avatarUrl]
            : [...scope.row.avatarUrl.split(',').map((item)=>baseUrl+'/files/'+item)]"
                      :src="scope.row.avatarUrl.indexOf('http')>-1?scope.row.avatarUrl
                      :baseUrl+'/files/'+scope.row.avatarUrl.split(',')[0]"></el-image>
          </template>
        </el-table-column>
...
<script>
import {fetchList, deleteById, saveOrUpdate} from '@/api/wxUser';
