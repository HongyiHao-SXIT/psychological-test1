  ...
  <div class="app-container">
    ...
    <div class="table-container">
      <el-table ref="roleTable"
        ...
        <el-table-column label="详情轮播图" align="center">
          <template slot-scope="scope">
            <el-image :preview-src-list="[...scope.row.bannerUrl.split(',').map((item)=>baseUrl+'/files/'+item)]"
                      :src="baseUrl+'/files/'+scope.row.bannerUrl.split(',')[0]"></el-image>
          </template>
        </el-table-column>
        ...
        <el-table-column label="擅长" align="center">
          <template slot-scope="scope">{{ scope.row.goodAt }}</template>
        </el-table-column>
        <el-table-column label="从业时间" align="center">
          <template slot-scope="scope">{{ scope.row.workTime }}</template>
        </el-table-column>
        <el-table-column label="咨询时间" align="center">
          <template slot-scope="scope">{{ scope.row.consultTime }}</template>
        </el-table-column>
        <el-table-column label="自我介绍" align="center">
          <template slot-scope="scope">{{ scope.row.intro }}</template>
        </el-table-column>
        <el-table-column label="咨询等级" align="center">
          <template slot-scope="scope">{{ scope.row.levelText }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            <el-button size="mini"
                       ...
                       @click="changeStatus(scope.$index, scope.row,'b')">通过
            </el-button>
            <el-button size="mini"
                       type="text"
                       :disabled="scope.row.status!=='a'"
                       @click="changeStatus(scope.$index, scope.row,'c')">拒绝
            </el-button>
            <el-button size="mini"
                       type="text"
    ...
    <el-dialog
      ...
      <el-form :model="massageItem"
        ...
        <el-form-item label="新闻标签">
          <el-input v-model="massageItem.tag" style="width: 250px"></el-input>
        </el-form-item>
...
<script>
import {fetchList, deleteById, saveOrUpdate} from '@/api/wxCounselor';
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      ...
      this.massageItem.bannerUrlList = []
      setTimeout(() => {
    ...
    handleUpdate(index, row) {
      ...
      this.massageItem.bannerUrlList = row.bannerUrl.split(',').map(item => {
        return {
          name: item,
          url: this.baseUrl + '/files/' + item,
          response: {
